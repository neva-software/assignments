...
1. Во время разработки приложения не стоит изобретать велосипеды. Для компонентов формы и карточек предусмотрена библиотека `material-ui`.
...
1. При разработке использовать статическую типизацию и другие возможности `typescript` (желательно, но не обязательно)
1. При перезапуске backend-части приложения список пользователей сбрасывается к стандартному состоянию!
1. Написанные unit/e2e-тесты будут плюсом.
...
Данный эндпоинт принимает запросы на регистрацию пользователей.
Метод HTTP-запроса - `POST`
